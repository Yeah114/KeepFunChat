...
options = Values({'spread': 3.0, 'freq': random.uniform(*config.get("彩色频率区间", [0.5, 1.0])), 'seed': 0, 'animate': False, 'duration': 12, 'speed': 20.0, 'force': False, 'mode': 256, 'charset_py2': 'utf-8', 'os': random.randint(*config.get("彩色初始区间", [0, 1000]))})
if os.name == 'nt':
    lolcat = LolCat(mode=options.mode,output=stdoutWin())
else:
    lolcat = LolCat(mode=options.mode)